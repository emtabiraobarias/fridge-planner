...
- **agent (automated, since 2026-07-27 — user-authorised):** **rolling out an approved release.** `scripts/deploy-release.sh <version>` calls Portainer's git-redeploy API with `pullImage:true`, `prune:false`, then verifies the served version via `/api/health` and fails if it did not land. Runs from a machine on the LAN (Actions cannot reach Portainer). Preconditions before running it: the version is **tagged**, its CI is **green**, and the user has **approved that release** — automation covers the rollout, not the decision to ship. Read-only `--check` first when anything about the stack may have changed. Rollback is the same script with the previous version.
