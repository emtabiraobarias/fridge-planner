...
async function create(overrides: Record<string, unknown> = {}, userId = 'u1'): Promise<string> {
  const res = await POST(
    req('/api/v1/inventory', { method: 'POST', body: { ...validItem, ...overrides }, userId }),
  );
  ...
  it('creates an item and computes expirationStatus from expiresAt', async () => {
    ...
    const res = await POST(
      req('/api/v1/inventory', {
        method: 'POST',
        body: { ...validItem, expiresAt: tomorrow.toISOString() },
      }),
    );
...
describe('Route Handler: PUT /api/v1/inventory/[id]', () => {
  it('updates the owner’s item', async () => {
    ...
    const res = await PUT(
      req(`/api/v1/inventory/${id}`, { method: 'PUT', body: { quantity: 5 } }),
      {
  ...
  it('updates location and expiry, recomputing expirationStatus (FR-002 / FR-UI-019 revised)', async () => {
    ...
    const res = await PUT(
      req(`/api/v1/inventory/${id}`, {
        method: 'PUT',
        body: { location: 'freezer', expiresAt: tomorrow },
      }),
      { params: Promise.resolve({ id }) },
    );
    expect(res.status).toBe(200);
  ...
  it('clears the expiry with expiresAt:null → status "none" (FR-002 / FR-UI-019 revised)', async () => {
    ...
    const res = await PUT(
      req(`/api/v1/inventory/${id}`, {
        method: 'PUT',
        body: { expiresAt: null, location: 'pantry' },
      }),
      { params: Promise.resolve({ id }) },
    );
    expect(res.status).toBe(200);
  ...
  it('returns 404 when updating another user’s item (isolation)', async () => {
    ...
    const res = await PUT(
      req(`/api/v1/inventory/${id}`, { method: 'PUT', body: { quantity: 5 }, userId: 'u1' }),
      {
        params: Promise.resolve({ id }),
      },
    );
  ...
  it('returns 400 for a malformed ObjectId', async () => {
    const res = await PUT(
      req('/api/v1/inventory/not-an-id', { method: 'PUT', body: { quantity: 5 } }),
      {
