...
const baseBug: IFeedbackRecord = {
  ...
  userStory:
    'As a home cook, I want the grocery count to match the items, so that I can trust the list.',
  acceptanceCriteria: [
    {
      given: 'a grocery list with 5 items',
      when: 'the user opens the grocery page',
      then: 'the header shows 5',
    },
    {
      given: 'an empty grocery list',
      when: 'the user opens the grocery page',
      then: 'the header shows 0',
    },
  ],
  ...
  acceptanceCriteria: [
    {
      ...
      then: 'the UI switches to a dark palette',
    },
  ],
  reproSteps: [],
  expectedBehavior: '',
...
describe('renderFeedbackMarkdown (FR-F-007 / SC-F-003 spec-template alignment)', () => {
  ...
  it('renders the user story prose and numbered Given/When/Then scenarios', () => {
    ...
    expect(md).toContain(
      '1. **Given** a grocery list with 5 items, **When** the user opens the grocery page, **Then** the header shows 5',
    );
    expect(md).toContain(
      '2. **Given** an empty grocery list, **When** the user opens the grocery page, **Then** the header shows 0',
    );
  });
