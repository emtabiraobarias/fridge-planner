...
**▶ NEXT ACTION:** **🎉 Phase D (authentication) COMPLETE on BOTH impls (2026-06-27).** Real OIDC/JWT auth (spec `002`) replaces the `X-User-Id` dev stub: validate Bearer JWT via JWKS + iss/aud/exp, identity from `sub`, FR-036 isolation (cross-user → 404), 401 Problem JSON, public `/health`, `dev`/`oidc` seam (`AUTH_MODE`) + prod guard. **`impl/nextjs`** (`3d0a3bb`+`69a415e`) verifies in the Next server layer (`authenticate()` + `withRoute`→401, 12 handlers); **`impl/vite`** (`40414d2`) verifies in the single Express `authMiddleware` (+`errorHandler`→401). Both: same shared spec + `jose` + env + client `AuthBanner` 401 UX; suites green (nextjs 277, vite server 216 + client 117), lint/build clean, **E2E gates 9/9 both**. Full spec-first chain done: spec(main) → plan → tasks → analyze (caught FR-D-009) → TDD code. **No phase queued.** Candidate next directions: **(A)** real prod-auth deployment (wire a live OIDC IdP + login UI — out of scope for `002`); **(B)** the Phase 2+ deferrals (true 202+poll async recs, quantity-aware grocery SG-03, checkout-time consumption); **(C)** add an auth step to the shared `scripts/smoke-test.sh`; **(D)** CI (run `validate-e2e --no-agent` + suites).
