...
**▶ NEXT ACTION:** **Phase B COMPLETE on BOTH impls** (impl/nextjs walked 4 areas; impl/vite confirmed 2026-06-11 by code-identity). SG-01/02/03 decided + applied to `spec.md`. **Move to Phase C — fix the triaged bugs (1 issue/session), routed per branch.** Suggested order: **#1 isolation** (cheap, HIGH, both branches — add `userId` filter to inventory GET/PUT/DELETE + recs query, then verify vs **FR-036**) → **#6 stale `expirationStatus`** (derive on read) → **#4** agent timeout/fallback (impl/vite worse) → **#7** consumption restore → **#2/#5**. Then the **async recommendation UX** (SC-002, per-branch client). Backend fixes land on one branch then cherry-pick to the other. Spec is now stable.
