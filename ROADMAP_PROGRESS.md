...
**▶ NEXT ACTION:** **Phase C-bis IN PROGRESS (`impl/nextjs`-only) — Cb0–Cb3 ✅ (all CRUD on Next; only recommendations left; 2026-06-24).** Express→Next Route Handler migration. **Cb0** scaffolded the Next server layer (`src/server/db.ts` globalThis-cached Mongoose, deps, `@server/*` alias; `e57e82a`). **Cb1** inventory (thin handlers over extracted controller + `http.ts`; node-env + `mongodb-memory-server` harness; `fad36c7`). **Cb2** grocery-lists — 6 routes under `[weekStart]/` incl. `complete`→inventory FR-032; copied grocery libs + grocery-list/meal-plan models; 16 tests; `71c5214`. **Cb3** meal-plans — GET/`entries` POST/`entries/[slotId]` DELETE/PUT over `controllers/meal-plans.ts`, preserving reversible consume/restore (BUG #7/FR-005); added `src/server/logger.ts` (framework-neutral, pino-shaped) to **decouple `ingredient-consumption` from Express's `app.ts`**; 11 tests drive REAL consume/restore vs seeded inventory; `e395890`. **Transition rule:** shared models/libs are **copied** into `src/server` (Express's recommendations route still needs them) → single source of truth when Express is deleted at **Cb5**. Coverage gate restored earlier (78.41→81.29%; `d475dc4`→`4f8132e`). Suites green: server **199/199**, client **154/154**, lint clean. **Next — Cb4 (recommendations, LAST):** the only endpoint with an external dependency (Holodeck `:8001`) — migrate the meal-recommender service + recommendations-cache + popular-recipes into `src/server` (`holodeck.ts`), preserve the popular/cache fallback + rate limit + long timeout. Spend the testing budget here (timeouts, agent-down, malformed responses). Then **Cb5** deletes Express. Per-step pattern: copy lib→`src/server` · thin handler over extracted controller · re-point proxy · node-env handler tests · both green.
...
**⏸ LAST LEFT OFF:** 2026-06-24 — **Phase C-bis Cb3** (meal-plans → Route Handlers; 4 routes over `controllers/meal-plans.ts`; reversible consume/restore preserved; added framework-neutral `src/server/logger.ts` to decouple `ingredient-consumption` from Express; 11 node-env handler tests driving real consume/restore; `e395890`). Client 143→154, server **199/199** untouched, lint clean. **All CRUD endpoints now on Next; only recommendations proxies to Express. Next: Cb4** (recommendations, last — external Holodeck dep). This ROADMAP edit landed on `main` and synced to both impls. *(Earlier this stretch: Cb0–Cb2 + coverage-gate restoration; prior session 2026-06-22: Phase C COMPLETE — see session log + spec-gap register below.)*
...
- **Phase C-bis — Retire Express into Next Route Handlers** (optional architectural change; sequenced against `002`) — ◐ **IN PROGRESS (2026-06-24): Cb0–Cb3 ✅ (all CRUD on Next); Cb4 (recommendations) next**, **`impl/nextjs`-only**
...
- [ ] Cb4 — Migrate **recommendations LAST**: `src/server/holodeck.ts` calling the sidecar on :8001. Spend the testing budget here — timeouts, agent-down, malformed responses. Only endpoint with an external dependency. ◀ NEXT. (Also move recommendations-cache — already in `src/server` since Cb1 — + popular-recipes + the rate limiter; preserve popular/cache fallback + 220s timeout.)
