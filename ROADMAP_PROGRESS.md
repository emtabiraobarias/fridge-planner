...
**⏸ LAST LEFT OFF:** 2026-06-24 — **Phase C-bis Cb4** (recommendations → Route Handler, last endpoint; `controllers/recommendations.ts` preserves the EC-01/cache/EC-08 fallback ladder; copied meal-recommender + popular-recipes; removed the next.config proxy entirely; 5 node-env tests w/ mocked agent; `f627c48`). Client 154→159, server **199/199** untouched, lint clean. **ALL endpoints now on Next; the Express proxy is gone (Express still runs but is unreachable until Cb5 deletes it). Next: Cb5** (retire Express — ⚠ rate-limiter not yet ported; confirm 2 teardown decisions). This ROADMAP edit landed on `main` and synced to both impls. *(Earlier this stretch: Cb0–Cb3 + coverage-gate restoration; prior session 2026-06-22: Phase C COMPLETE — see session log + spec-gap register below.)*
...
- **Phase C-bis — Retire Express into Next Route Handlers** (optional architectural change; sequenced against `002`) — ◐ **IN PROGRESS (2026-06-24): Cb0–Cb4 ✅ (ALL endpoints on Next, proxy gone); Cb5 (retire Express) next**, **`impl/nextjs`-only**
...
- [ ] Cb5 — Retire Express: ~~delete proxy rules from `next.config.ts`~~ (done in Cb4); remove `packages/server` from dev script + docker-compose; drop `CORS_ORIGIN` (same-origin now); **port remaining middleware → the recommendations 10-req/min rate limiter (currently unthrottled) + a shared error wrapper (unhandled throw → Problem JSON)**; then delete `packages/server` (single source of truth). One Node process. ◀ NEXT. ⚠ Confirm: (1) rate-limiter approach; (2) delete `packages/server` vs keep its Jest suite.
