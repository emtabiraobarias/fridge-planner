...
**▶ NEXT ACTION:** **Phase C in progress** (1 issue/session, backend fixes led on `impl/vite` → cherry-pick to `impl/nextjs`). **✔ Done: #1 isolation** (FR-036; TDD `isolation.test.ts`; both branches). **Next: #6 stale `expirationStatus`** — derive status on read instead of persisting it (model/read path; breaks SC-014 + stale UI). Then **#4** agent timeout/fallback (⚠ `impl/vite` lacks the 220s timeout — add it there), **#7** consumption restore, **#2/#5**, then the **async recommendation UX** (SC-002, per-branch client). Pattern proven: red test → fix → green (174/174) → lint → cherry-pick → both green.
