...
**▶ NEXT ACTION:** **Phase C-bis IN PROGRESS (`impl/nextjs`-only) — Cb0 ✅ + Cb1 ✅ (2026-06-23).** Express→Next Route Handler migration underway. **Cb0** scaffolded the Next server layer (`src/server/db.ts` `globalThis`-cached Mongoose, `mongoose`/`zod`/`server-only` deps, `@server/*` alias; `e57e82a`). **Cb1** migrated **inventory** to Route Handlers — thin `app/api/v1/inventory/{route,[id]/route}.ts` over an extracted framework-agnostic controller (`src/server/controllers/inventory.ts` + `http.ts`), proxy re-pointed (explicit allow-list, not catch-all, so it can't shadow `/inventory/[id]`), tested against real Mongo (node-env Vitest + `mongodb-memory-server`; `@server`+`server-only` aliased); `fad36c7`. **Transition rule:** shared models/libs are **copied** into `src/server` (Express's not-yet-migrated routes still depend on them) → single source of truth when Express is deleted at **Cb5**. Also **restored the server 80% branch-coverage gate** — latently red at 78.41% from undertested Phase C #6/#7 branches (`expirationStatusQuery` arms, `restoreIngredients`) → **81.29%**; test-only, lead `impl/vite` `d475dc4` → cherry-pick `impl/nextjs` `4f8132e`. Suites green BOTH branches: server **199/199**, client (nextjs) **127/127**, `npm test` exit 0. **Next — Cb2** (grocery-lists incl. `/generate`, `/items`, `/complete` checkout→consume). Known wrinkle: `ingredient-consumption.ts` imports `logger` from Express `app.ts` → needs a framework-neutral logger in `src/server` (shared with Cb3). Per-step pattern: copy lib→`src/server` · thin handler over extracted controller · re-point proxy · node-env handler tests · both green.
...
**⏸ LAST LEFT OFF:** 2026-06-23 — **Phase C-bis Cb0 + Cb1** (Next server-layer scaffold; inventory → Route Handlers with the reusable node-env handler-test harness) **+ server coverage-gate restoration** (Phase C #6/#7 branch tests, 78.41→81.29%). All committed; `npm test` green on BOTH branches (server **199/199**, client **127/127**). Phase C-bis test/migration pattern now proven → Cb2–Cb4 are mechanical repeats. **Next: Cb2** (grocery-lists + checkout). This ROADMAP edit landed on `main` and synced to both impls. *(Prior session 2026-06-22: Phase C COMPLETE — all 7 bugs + SG-01/02/03 resolved on both impls; see session log + spec-gap register below.)*
...
- **Phase C-bis — Retire Express into Next Route Handlers** (optional architectural change; sequenced against `002`) — ◐ **IN PROGRESS (2026-06-23): Cb0 ✅ Cb1 ✅; Cb2 next**, **`impl/nextjs`-only**
...
- [ ] Cb2 — Migrate **grocery-lists**, including nested `checkout` action as its own `route.ts`. ◀ NEXT. (Pulls in grocery libs + the grocery model; decouple `ingredient-consumption.ts` from Express's `logger`.)
