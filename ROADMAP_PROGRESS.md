...
**▶ NEXT ACTION:** **Phase C-bis IN PROGRESS (`impl/nextjs`-only) — Cb0 ✅ Cb1 ✅ Cb2 ✅ (3 of 4 endpoints on Next; 2026-06-24).** Express→Next Route Handler migration. **Cb0** scaffolded the Next server layer (`src/server/db.ts` globalThis-cached Mongoose, deps, `@server/*` alias; `e57e82a`). **Cb1** migrated **inventory** (thin handlers over extracted controller + `http.ts`; proxy explicit allow-list; node-env + `mongodb-memory-server` test harness; `fad36c7`). **Cb2** migrated **grocery-lists** — all six routes under `app/api/v1/grocery-lists/[weekStart]/` (GET lazy-gen · `generate` · `items` POST · `items/[itemId]` PATCH/DELETE · `complete`→inventory FR-032) over `controllers/grocery-lists.ts`; copied the grocery libs (generator/matcher/categorizer/unit-normalizer) + grocery-list & meal-plan models; 16 handler tests; `71c5214`. **Transition rule:** shared models/libs are **copied** into `src/server` (Express's not-yet-migrated routes still need them) → single source of truth when Express is deleted at **Cb5**. Coverage gate restored earlier (78.41→81.29%; `d475dc4`→`4f8132e`). Suites green: server **199/199**, client **143/143**, lint clean. **Next — Cb3** (meal-plans read/write + reversible consume/restore). Known wrinkle now confirmed Cb3-scoped: `ingredient-consumption.ts` imports `logger` from Express `app.ts` → decouple into a framework-neutral `src/server` logger. Per-step pattern: copy lib→`src/server` · thin handler over extracted controller · re-point proxy · node-env handler tests · both green.
...
**⏸ LAST LEFT OFF:** 2026-06-24 — **Phase C-bis Cb2** (grocery-lists + checkout → Route Handlers; 6 routes over `controllers/grocery-lists.ts`; 16 node-env handler tests; `71c5214`). Client 127→143, server **199/199** untouched, lint clean. Learned: `complete` ADDS to inventory (no `ingredient-consumption`) → the `logger` decoupling is a **Cb3** task, not Cb2. **Next: Cb3** (meal-plans). This ROADMAP edit landed on `main` and synced to both impls. *(Earlier this stretch: Cb0+Cb1 + coverage-gate restoration; prior session 2026-06-22: Phase C COMPLETE — see session log + spec-gap register below.)*
...
- **Phase C-bis — Retire Express into Next Route Handlers** (optional architectural change; sequenced against `002`) — ◐ **IN PROGRESS (2026-06-24): Cb0 ✅ Cb1 ✅ Cb2 ✅; Cb3 next**, **`impl/nextjs`-only**
...
- [ ] Cb3 — Migrate **meal-plans** read/write only (NOT recommendations yet). ◀ NEXT. (Reversible consume/restore on POST/PUT/DELETE → **decouple `ingredient-consumption.ts` from Express's `logger`** into a framework-neutral `src/server` logger.)
