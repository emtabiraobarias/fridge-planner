...
- **FR-UI-019**: The quantity stepper MUST step by 50 for g/ml, 0.5 for kg/L, and 1 otherwise; decrementing an item to zero MUST remove it. Inventory edit-in-place (the former per-field edit) is removed — the stepper and delete cover item mutation.
...
- **FR-UI-031**: The following existing flows are intentionally changed: inventory edit-in-place is **removed** (FR-UI-019); the grocery checkout confirmation modal is **replaced** by inline checkout (FR-UI-029); drag-and-drop meal placement is **demoted** to an optional enhancement behind tap-to-place (FR-UI-026). Their removal MUST NOT break the underlying API behaviour they used.
