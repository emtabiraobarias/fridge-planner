...
- **FR-F-007**: *(moved to `012` — see `FR-FL-066`, and `FR-FL-032`/`FR-FL-033` for the richer brief.)* Export is still available for **any** `complete` record (`FR-FL-066`), and is administrator-only; brief assembly at `briefed` builds on it once clauses are vetted. *Corrected 2026-08-24: this pointer originally named only the brief-assembly requirements, which narrowed export to records that had reached `briefed`.* Retained as a pointer.
