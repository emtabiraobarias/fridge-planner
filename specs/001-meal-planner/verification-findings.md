...
| SC-002 | Recs | ☑ pass | — | — | **Resolved via SG-02 (`9a2c33e`)** — SC-002 reworded to async/time-to-first-feedback; client gives immediate `Thinking…` + skeletons (non-blocking) and a fallback notice. Cold latency no longer a violation (async). True 202+poll deferred Phase 2+ |
...
| 3 | SC-002 | **Recommendation latency ~28× over budget** — cold call measured **142 s** vs the **5 s** SC-002 target. Root cause is the deliberate web-searching agent (`WebSearch`+`WebFetch`, `max_turns:15`). Cache (15 min TTL) only helps identical repeat calls; any inventory mutation invalidates it, so normal use is mostly cold. | **HIGH** | **✔ RESOLVED 2026-06-22 (SG-02 async)** — SC-002/FR-012 reworded to async (decided: async UX + soften SC-002, not perf re-architecture). Client surfaces the `fallback` flag + immediate non-blocking loading (`b193323`→`9a2c33e`). The 142s cold call is acceptable under the async wording. **True 202+poll job model deferred to Phase 2+.** |
