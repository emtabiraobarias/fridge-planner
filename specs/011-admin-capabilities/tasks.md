...
- [ ] T067 [P] *(deferred with the release — the Keycloak step is documented in quickstart.md meanwhile)* Cascade `docs/deployment.md`: the **manual** Keycloak realm-role step, the two new env vars, and the reminder that `AUTH_ALLOW_DEV` must stay absent in production
