...
- [X] T070 Release the agent — **done 2026-08-30**: `agent-feedback-v1.2.0` (annotated, on `b7e63f8`), image `sha256:0d46c8fc…`, pinned via PR #87. Released BEFORE the app on purpose (research R3) so the app never calls a clause mode the agent does not know; until 4.15.1 landed, drafting degraded to zero clauses. 1.2.0 rather than 1.1.1 because `agent-feedback-v1.1.0` is the broken Claude-era build prod skips — and it turns out to be tagged on a commit that does not contain 1.0.1, the 4.14.0 shape exactly.
...
- [X] T072 Cut the app release — **done 2026-08-30**: `nextjs-v4.15.1` via `scripts/cut-release.sh`, image `sha256:f93692ca…`, pinned via PR #89, rolled in ~5 min. `verify-rollout.sh 4.15.1` ✅ and `/api/v1/admin/lifecycle` flipped 404 → 401, which is the probe that actually proves the new routes serve. **No 4.15.0 image**: that tag was rejected by a bug in the release guard itself (`actions/checkout` rewrites the triggering ref as a commit, so every annotated tag looked lightweight); fixed in `ae2e09a`, tag left un-repointed. Prod's `feedbackrecords`/`pipelineitems` were flushed by the operator first, so the `approved → accepted` migration (T008) was never needed there. Verified in prod: clause drafting returns clauses, and a closed item shows the reporter "Done" + excerpt + release link.
