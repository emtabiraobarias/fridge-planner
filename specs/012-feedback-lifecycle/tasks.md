...
- [ ] T033 [P] [US3] Add the clause-drafting mode to `agents/feedback-collector/instructions/system-prompt.md` — returns `{status:'clauses', clauses:[{text, derivedFrom, inferred}]}`, one trigger and one response per clause, **derive only, never invent** (FR-FL-026/030)
- [ ] T034 [P] [US3] Add a `ClauseDraftQuality` eval + test cases to `agents/feedback-collector/agent.yaml`
- [ ] T035 [US3] **(RED)** `packages/client/tests/server/unit/feedback-collector-clauses.test.ts` — third member of the Zod discriminated union parses; fence-stripped and prose-wrapped JSON salvaged; malformed → throws
- [ ] T036 [US3] **(GREEN)** Extend `packages/client/src/server/services/feedback-collector.ts` with the clause mode (research R3). Must **fail soft** when the agent predates the mode — FR-FL-031 already makes "no clauses drafted" a first-class path
- [ ] T037 [P] [US3] **(RED)** `packages/client/tests/server/lifecycle-clauses.test.ts`: drafting on entering `briefed` (FR-FL-024); `derivedFrom` required (FR-FL-025); `inferred` marked (FR-FL-026); provisional ids, never `FR-` (FR-FL-027); **`briefed→in-spec` refused while any clause pending** (FR-FL-028, SC-FL-005); per-clause accept/edit/reject (FR-FL-029); manual authoring when drafting yields nothing (FR-FL-031); rate-limited on the shared `feedback-chat:${userId}` bucket
- [ ] T038 [US3] **(GREEN)** Clause storage + vetting in `packages/client/src/server/controllers/lifecycle.ts`
