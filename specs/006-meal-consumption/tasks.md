...
- [x] T029 [P] Live agent smoke (optional, needs `OPENAI_API_KEY`): `docker compose up -d --build holodeck`, request recommendations, verify grounded output shape end-to-end; run the `agent.yaml` eval case if credentials allow
...
- [x] T031 Release prep (with the user): version bump, tag `nextjs-v4.4.0` + `agent-v*` (prompt change requires the sidecar image), CI build-push, manual Portainer Pull-and-redeploy, roadmap update on `main`
