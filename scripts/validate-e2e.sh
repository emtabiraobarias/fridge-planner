#!/usr/bin/env bash
#
# One-shot E2E validation for impl/vite.
#
# Boots the REAL backend — a production build of the Express API (`tsc` +
# `node dist/index.js`), a real MongoDB (throwaway DB), and optionally the live
# Holodeck agent — runs the shared scripts/smoke-test.sh against it (:3001), then
# tears everything down. Exit code = smoke result.
#
# This is the per-branch BOOT WRAPPER. The smoke STEPS live in the shared
# scripts/smoke-test.sh (on main). impl/nextjs has its own validate-e2e.sh that boots
# one Next process and points BASE at :3000 instead.
#
# The API smoke is transport-only (curl), so it needs the Express API on :3001 — not
# the Vite SPA.
#
...
echo "[2/4] production build (tsc)"
npm -w packages/server run build || { echo "❌ build failed"; exit 1; }

echo "[3/4] start Express (node dist) on :$PORT against $DB"
# AUTH_MODE=dev: run the X-User-Id dev seam so the smoke round-trips without a live IdP (FR-D-007).
PORT="$PORT" MONGODB_URI="mongodb://localhost:27017/$DB" HOLODECK_URL="http://localhost:8001" \
  AUTH_MODE=dev \
  npm -w packages/server run start >/tmp/e2e-app.log 2>&1 &
APP_PID=$!
ready=0
for _ in $(seq 1 60); do
  # readiness via the PUBLIC /health endpoint (no auth) — FR-D-006
  curl -sf -o /dev/null "http://localhost:$PORT/health" && { ready=1; break; }
  sleep 1
done
